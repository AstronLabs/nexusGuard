...
import type { PoolSummary, Claim, ClaimStatus, PoolStatus, Role } from "./types";
...
function parsePoolStatus(raw: any): PoolStatus {
  if (typeof raw === "string") return raw as PoolStatus;
  const key = typeof raw === "object" ? Object.keys(raw)[0] : String(raw);
  const statusMap: Record<string, PoolStatus> = {
    Active: "Active" as PoolStatus,
    Paused: "Paused" as PoolStatus,
    Matured: "Matured" as PoolStatus,
    Closed: "Closed" as PoolStatus,
  };
